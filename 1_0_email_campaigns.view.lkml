    ...
    LANGUAGE js AS "return decodeURIComponent(a)";
      ...
      from(
        select
          e.marketing_campaign_id
          ,e.marketing_campaign_name
          ,upper(split(split(e.url,'utm_source=')[safe_offset(1)],'&')[safe_offset(0)]) as source
          ,upper(split(split(e.url,'utm_medium=')[safe_offset(1)],'&')[safe_offset(0)]) as medium
          ,upper(split(split(e.url,'utm_campaign=')[safe_offset(1)],'&')[safe_offset(0)]) as campaign
          ,upper(split(split(e.url,'utm_content=')[safe_offset(1)],'&')[safe_offset(0)]) as content
          ,decodeurl(upper(split(split(e.url,'utm_term=')[safe_offset(1)],'&')[safe_offset(0)])) as term
