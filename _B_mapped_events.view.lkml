...
view: mapped_events {
  derived_table: {
    ...
    sql: select *
        ...
        ,IF(e.event_source='pages' AND e.event NOT IN ('Product'), e.event, LAST_VALUE(IF(e.event_source='pages' AND e.event NOT IN ('Product'), e.event, NULL) IGNORE NULLS) OVER (PARTITION BY e.anonymous_id ORDER BY e.timestamp ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING)) AS journey_type
